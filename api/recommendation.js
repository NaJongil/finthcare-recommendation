export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'verify') {
      ...
      const normalizedPlannerPhone = plannerPhone.replace(/[^0-9]/g, '');

      // RequesterPhone, PatientPhone, PlannerPhone 중 하나라도 일치하면 인증 성공
      if (inputPhone === normalizedRequesterPhone || inputPhone === normalizedPatientPhone || inputPhone === normalizedPlannerPhone) {
        // 인증 성공 - 전체 데이터 반환
        return res.status(200).json({
          success: true,
          data: {
